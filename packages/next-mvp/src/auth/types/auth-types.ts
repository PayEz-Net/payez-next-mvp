 ...
 */
export type RedisSessionId = string & { readonly __brand: 'RedisSessionId' };
 ...
 */
export type IdpAccessToken = string & { readonly __brand: 'IdpAccessToken' };
 ...
 * - token.refreshToken
 */
export type IdpRefreshToken = string & { readonly __brand: 'IdpRefreshToken' };
 ...
 * - oauthAccessToken
 */
export type OAuthProviderToken = string & { readonly __brand: 'OAuthProviderToken' };

// ============================================================================
// HELPER FUNCTIONS - Create branded types from raw strings
// ============================================================================

export function toRedisSessionId(value: string): RedisSessionId {
  return value as RedisSessionId;
}

export function toIdpAccessToken(value: string): IdpAccessToken {
  return value as IdpAccessToken;
}

export function toIdpRefreshToken(value: string): IdpRefreshToken {
  return value as IdpRefreshToken;
}

export function toOAuthProviderToken(value: string): OAuthProviderToken {
  return value as OAuthProviderToken;
}
...
export interface RedisSessionData {
  ...
  // -------------------------------------------------------------------------
  // IDP Tokens - THE source for API authorization
  // -------------------------------------------------------------------------
  ...
  // -------------------------------------------------------------------------
  // OAuth Provider Info (if user signed in via OAuth)
  // -------------------------------------------------------------------------
...
// ============================================================================
// APP SESSION - What the frontend sees
// ============================================================================
...
export type EnsureFreshTokenResult =
  | { success: true; idpAccessToken: IdpAccessToken; sessionData: RedisSessionData }
...
export type AuthError =
  | 'NO_SESSION'        // No session cookie or Redis entry
  | 'NO_TOKEN'          // Session exists but no access token
  | 'TOKEN_EXPIRED'     // Access token expired and refresh failed
  | 'REFRESH_FAILED'    // Could not refresh the token
  | 'MFA_REQUIRED'      // User needs to complete MFA
  | 'MFA_EXPIRED'       // MFA verification has expired
  | 'SESSION_INVALID'   // Session data is corrupted or invalid
  | 'CLIENT_MISMATCH';  // Token was issued for a different client
